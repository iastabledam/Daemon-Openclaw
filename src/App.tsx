...
const COST_HTML = `<!DOCTYPE html>
<html lang="fr">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>OpenClaw — Optimisation des Coûts</title>
...
/* NAV */
.nav { position:sticky; top:0; z-index:100; background:rgba(9,9,11,0.85); backdrop-filter:blur(20px); border-bottom:1px solid var(--border); }
.nav-inner { max-width:1100px; margin:0 auto; padding:0 24px; display:flex; align-items:center; gap:8px; height:52px; overflow-x:auto; scrollbar-width:none; }
.nav-inner::-webkit-scrollbar { display:none; }
.nav-link { color:var(--muted); font-size:12px; font-weight:500; white-space:nowrap; padding:4px 10px; border-radius:6px; cursor:pointer; text-decoration:none; transition:all 0.15s; border:none; background:transparent; font-family:'Inter',sans-serif; }
.nav-link:hover { color:var(--text); background:rgba(255,255,255,0.05); }
.nav-link.active { color:var(--amber); background:rgba(245,158,11,0.1); }
.nav-sep { width:1px; height:14px; background:var(--border2); flex-shrink:0; }

/* HERO */
.hero { position:relative; padding:72px 24px 56px; text-align:center; overflow:hidden; }
.hero::before { content:''; position:absolute; top:-40%; left:50%; transform:translateX(-50%); width:80%; height:80%; background:radial-gradient(ellipse, rgba(245,158,11,0.1) 0%, transparent 70%); pointer-events:none; }
.hero-badge { display:inline-flex; align-items:center; gap:6px; padding:4px 14px; border-radius:100px; background:rgba(245,158,11,0.1); border:1px solid rgba(245,158,11,0.25); color:var(--amber); font-size:11px; font-weight:600; letter-spacing:0.08em; text-transform:uppercase; font-family:'JetBrains Mono',monospace; margin-bottom:20px; }
.hero h1 { font-size:clamp(2rem,5vw,3.2rem); font-weight:800; line-height:1.1; margin-bottom:12px; letter-spacing:-0.03em; }
.hero h1 span { background:linear-gradient(135deg,#f59e0b,#fbbf24,#f97316); -webkit-background-clip:text; -webkit-text-fill-color:transparent; background-clip:text; }
.hero-sub { color:var(--muted); font-size:14px; margin-bottom:36px; }

/* SAVINGS BANNER */
.savings-row { display:flex; justify-content:center; gap:16px; flex-wrap:wrap; max-width:800px; margin:0 auto 0; }
.saving-card { background:var(--surface); border:1px solid var(--border2); border-radius:14px; padding:20px 24px; text-align:center; flex:1; min-width:160px; max-width:220px; position:relative; overflow:hidden; }
.saving-card::before { content:''; position:absolute; inset:0; background:linear-gradient(135deg, rgba(245,158,11,0.05), transparent); pointer-events:none; }
.saving-num { font-size:2rem; font-weight:800; font-family:'JetBrains Mono',monospace; color:var(--amber); line-height:1; margin-bottom:4px; }
.saving-label { font-size:12px; color:var(--muted); font-weight:500; }

/* CONTENT */
.content { max-width:860px; margin:0 auto; padding:48px 24px 80px; }

/* SECTION DIVIDER */
.section-title { text-align:center; font-size:11px; font-weight:700; letter-spacing:0.15em; text-transform:uppercase; color:var(--muted); margin:48px 0 24px; display:flex; align-items:center; gap:12px; }
.section-title::before, .section-title::after { content:''; flex:1; height:1px; background:var(--border); }

/* STEP CARD */
.step { background:var(--surface); border:1px solid var(--border); border-radius:16px; overflow:hidden; margin-bottom:16px; transition:border-color 0.2s; }
.step:hover { border-color:rgba(245,158,11,0.2); }
.step-header { display:flex; align-items:center; gap:14px; padding:18px 22px; border-bottom:1px solid var(--border); }
.step-num { width:32px; height:32px; border-radius:8px; background:rgba(245,158,11,0.12); border:1px solid rgba(245,158,11,0.25); display:flex; align-items:center; justify-content:center; font-size:13px; font-weight:700; color:var(--amber); font-family:'JetBrains Mono',monospace; flex-shrink:0; }
...
.tbl thead th { background:rgba(255,255,255,0.03); padding:10px 14px; text-align:left; color:var(--muted); font-weight:600; font-size:11px; text-transform:uppercase; letter-spacing:0.08em; border-bottom:1px solid var(--border); }
.tbl tbody td { padding:11px 14px; border-bottom:1px solid rgba(255,255,255,0.03); color:#a1a1aa; vertical-align:middle; }
.tbl tbody tr:last-child td { border-bottom:none; }
.tbl tbody tr:hover td { background:rgba(255,255,255,0.02); }

/* BADGES */
.badge { display:inline-flex; align-items:center; padding:2px 10px; border-radius:100px; font-size:11px; font-weight:600; font-family:'JetBrains Mono',monospace; white-space:nowrap; }
.badge-green { background:rgba(16,185,129,0.12); border:1px solid rgba(16,185,129,0.25); color:#34d399; }
...
.badge-blue { background:rgba(59,130,246,0.12); border:1px solid rgba(59,130,246,0.25); color:#93c5fd; }

/* CODE BLOCK */
.cblock { background:#07070a; border:1px solid rgba(255,255,255,0.07); border-radius:10px; overflow:hidden; margin:10px 0; }
.cblock-header { display:flex; align-items:center; justify-content:space-between; padding:8px 14px; border-bottom:1px solid rgba(255,255,255,0.05); }
.cblock-label { font-size:11px; color:var(--muted); font-family:'JetBrains Mono',monospace; }
.cblock-copy { background:rgba(255,255,255,0.05); border:1px solid var(--border); border-radius:4px; color:var(--muted); font-size:10px; padding:2px 8px; cursor:pointer; font-family:'JetBrains Mono',monospace; transition:all 0.15s; }
.cblock-copy:hover { color:var(--text); background:rgba(255,255,255,0.1); }
.cblock-code { padding:14px 16px; font-family:'JetBrains Mono',monospace; font-size:12px; color:#a5f3a0; line-height:1.7; overflow-x:auto; white-space:pre-wrap; word-break:break-all; }

/* CALLOUTS */
.callout { display:flex; gap:10px; padding:13px 16px; border-radius:10px; margin:12px 0; font-size:13px; line-height:1.6; }
.callout-warn { background:rgba(245,158,11,0.07); border:1px solid rgba(245,158,11,0.2); color:#fcd34d; }
.callout-success { background:rgba(16,185,129,0.07); border:1px solid rgba(16,185,129,0.2); color:#6ee7b7; }
.callout-info { background:rgba(59,130,246,0.07); border:1px solid rgba(59,130,246,0.2); color:#93c5fd; }

/* INLINE CODE */
.code { font-family:'JetBrains Mono',monospace; background:rgba(255,255,255,0.07); padding:2px 7px; border-radius:4px; font-size:0.85em; color:#e4e4e7; }

/* PARA */
.para { color:#a1a1aa; font-size:14px; line-height:1.75; margin:10px 0; }
.para strong { color:var(--text); }

/* H3 */
.h3 { font-size:13px; font-weight:700; color:var(--text); margin:18px 0 8px; }

/* SCREENSHOTS */
.screenshots { display:flex; gap:12px; flex-wrap:wrap; justify-content:center; margin:16px 0; }
.screenshot { border-radius:10px; overflow:hidden; border:1px solid var(--border2); max-width:220px; flex:1; min-width:140px; }
.screenshot img { width:100%; display:block; }
.screenshot-cap { font-size:11px; color:var(--muted); padding:6px 10px; text-align:center; background:rgba(255,255,255,0.02); font-family:'JetBrains Mono',monospace; }

/* FOOTER */
footer { text-align:center; padding:32px 24px 48px; color:var(--muted); font-size:12px; border-top:1px solid var(--border); }
footer a { color:var(--amber); text-decoration:none; }

@media(max-width:600px) { .savings-row { flex-direction:column; align-items:center; } .hero h1 { font-size:1.8rem; } }
</style>
</head>
<body>
...
<nav class="nav">
  <div class="nav-inner">
    ...
    <a class="nav-link" href="#step1">1. Auth</a>
    <a class="nav-link" href="#step2">2. Profil</a>
    <a class="nav-link" href="#step3">3. Modèles</a>
    <a class="nav-link" href="#step4">4. Défaut</a>
    <a class="nav-link" href="#step5">5. Alias</a>
    <div class="nav-sep"></div>
    <a class="nav-link" href="#step6">6. Heartbeat</a>
    <a class="nav-link" href="#step7">7. Compaction</a>
    <a class="nav-link" href="#step8">8. Cache</a>
    <a class="nav-link" href="#step9">9. Garde-fous</a>
    <a class="nav-link" href="#step10">10. Vérif</a>
    <div class="nav-sep"></div>
    <a class="nav-link" href="#ref">Référence</a>
    <a class="nav-link" href="#prompts">Prompts</a>
    <a class="nav-link" href="#debug">Dépannage</a>
  </div>
</nav>

<!-- HERO -->
<section class="hero">
  <div class="hero-badge">💰 Optimisation · Mars 2026</div>
  <h1>OpenClaw<br><span>Optimisation des Coûts</span></h1>
  <p class="hero-sub">Configuration OpenRouter sur VPS Hostinger / Coolify · De $2 550 à $70–95/mois</p>

  <div class="savings-row">
    <div class="saving-card">
      <div class="saving-num">-96%</div>
      <div class="saving-label">Économie totale</div>
    </div>
    <div class="saving-card">
      <div class="saving-num">-99%</div>
      <div class="saving-label">Sur les heartbeats</div>
    </div>
    <div class="saving-card">
      <div class="saving-num">-73%</div>
      <div class="saving-label">Modèle par défaut</div>
    </div>
    <div class="saving-card">
      <div class="saving-num">$70</div>
      <div class="saving-label">Coût cible/mois</div>
    </div>
  </div>
</section>
...
<div class="content">
  ...
  <div id="intro" class="step">
    ...
    <div class="step-body">
      <p class="para">Ce guide explique comment connecter <strong>OpenRouter</strong> à votre instance OpenClaw pour réduire drastiquement vos coûts en tokens.</p>
      <p class="para">Par défaut, OpenClaw utilise <strong>Claude Opus</strong> pour toutes les requêtes. Résultat : une facture pouvant dépasser <span class="badge badge-red">$2 500/mois</span> pour un usage actif.</p>

      <table class="tbl" style="margin-top:18px">
        <thead><tr><th>Optimisation</th><th>Coût avant</th><th>Coût après</th><th>Économie</th></tr></thead>
        <tbody>
          <tr><td>Modèle par défaut → DeepSeek V3.2</td><td><span class="badge badge-red">$2 550/mois</span></td><td><span class="badge badge-green">$680/mois</span></td><td><span class="badge badge-green">-73%</span></td></tr>
          <tr><td>Heartbeats → Gemini Flash-Lite</td><td><span class="badge badge-red">$1 022/mois</span></td><td><span class="badge badge-green">$7/mois</span></td><td><span class="badge badge-green">-99%</span></td></tr>
          <tr><td>Gestion de session + cache</td><td>—</td><td>—</td><td><span class="badge badge-green">-$60/mois</span></td></tr>
          <tr><td><strong>Total combiné</strong></td><td><span class="badge badge-red">$2 550/mois</span></td><td><span class="badge badge-green">$70–95/mois</span></td><td><span class="badge badge-amber">jusqu'à -96%</span></td></tr>
        </tbody>
      </table>

      <p class="para" style="margin-top:16px"><strong>Comment utiliser ce guide :</strong> Exécuter les commandes dans l'ordre depuis le <strong>Terminal Coolify</strong>. Un <strong>Restart</strong> est nécessaire après toutes les commandes.</p>
      <div class="callout callout-warn">⚠️ Ne pas envoyer de message à OpenClaw entre les commandes et le restart — cela pourrait écraser certains paramètres.</div>
    </div>
  </div>

  <div class="section-title">Configuration OpenRouter</div>

  <!-- STEP 1 -->
  <div id="step1" class="step">
    <div class="step-header">
      <div class="step-num">1</div>
      <div class="step-title">Enregistrer la clé OpenRouter</div>
    </div>
    <div class="step-body">
      <p class="para">Créer d'abord une clé API sur <strong>openrouter.ai → Dashboard → Keys → Créer une nouvelle clé</strong>.</p>
      <div class="cblock">
        <div class="cblock-header"><span class="cblock-label">Terminal Coolify</span><button class="cblock-copy" onclick="copy(this,'openclaw models auth paste-token --provider openrouter')">copier</button></div>
        <div class="cblock-code">openclaw models auth paste-token --provider openrouter</div>
      </div>
      <div class="callout callout-success">✅ Succès attendu : Auth profile: openrouter:manual (openrouter/token)</div>
    </div>
  </div>
  ...
  <div id="step2" class="step">
    <div class="step-header">
      ...
      <div class="step-title">Corriger le profil auth en mode api_key</div>
    </div>
    <div class="step-body">
      <p class="para">Par défaut le profil est créé en mode <span class="code">token</span>. Il faut le passer en <span class="code">api_key</span> pour que les modèles OpenRouter soient autorisés.</p>
      <div class="cblock">
        <div class="cblock-header"><span class="cblock-label">Terminal Coolify</span><button class="cblock-copy" onclick="copyEl(this,'code2')">copier</button></div>
        <div class="cblock-code" id="code2">openclaw config set auth.profiles.openrouter:manual '{"provider":"openrouter","mode":"api_key"}'</div>
      </div>
    </div>
  </div>
  ...
  <div id="step4" class="step">
    <div class="step-header">
      ...
      <div class="step-title">Définir le modèle par défaut</div>
    </div>
    <div class="step-body">
      <p class="para">DeepSeek V3.2 est le modèle économique recommandé <span class="badge badge-green">$0.31/M tokens</span>.</p>
      <div class="cblock">
        <div class="cblock-header"><span class="cblock-label">Terminal Coolify</span><button class="cblock-copy" onclick="copy(this,'openclaw config set agents.defaults.model.primary openrouter/deepseek/deepseek-v3.2')">copier</button></div>
        <div class="cblock-code">openclaw config set agents.defaults.model.primary openrouter/deepseek/deepseek-v3.2</div>
      </div>
    </div>
  </div>
  ...
  <div id="step5" class="step">
    <div class="step-header">
      ...
      <div class="step-title">Configurer les alias de modèles</div>
    </div>
    <div class="step-body">
      <p class="para">Les alias permettent de changer de modèle depuis Telegram avec <span class="code">/model deepseek</span>, <span class="code">/model sonnet</span>, etc.</p>
      <div class="cblock">
        <div class="cblock-header"><span class="cblock-label">Terminal Coolify</span><button class="cblock-copy" onclick="copyEl(this,'code5')">copier</button></div>
        <div class="cblock-code" id="code5">openclaw config set agents.defaults.models '{"openrouter/deepseek/deepseek-v3.2":{"alias":"deepseek"},"openrouter/google/gemini-2.5-flash-lite":{"alias":"flashlite"},"openrouter/google/gemini-2.5-flash":{"alias":"flash"},"openrouter/anthropic/claude-haiku-4-5":{"alias":"haiku"},"openrouter/anthropic/claude-sonnet-4-6":{"alias":"sonnet"},"openrouter/anthropic/claude-opus-4-6":{"alias":"opus"},"openrouter/moonshotai/kimi-k2.5":{"alias":"kimi"},"openrouter/z-ai/glm-5":{"alias":"glm"},"openrouter/openai/gpt-5-mini":{"alias":"gpt"},"openrouter/openai/gpt-5-nano":{"alias":"nano"},"meta-llama/llama-3.1-8b-instruct":{"alias":"llama8b"},"meta-llama/llama-3.3-70b-instruct":{"alias":"llama70b"},"meta-llama/llama-4-scout":{"alias":"llama4"}}'</div>
      </div>
    </div>
  </div>

  <div class="section-title">Optimisations des coûts</div>

  <!-- STEP 6 -->
  <div id="step6" class="step">
    <div class="step-header">
      <div class="step-num">6</div>
      <div class="step-title">Heartbeat économique</div>
    </div>
    <div class="step-body">
      <p class="para">Par défaut les heartbeats tournent sur Opus toutes les 30 minutes — soit <span class="badge badge-red">$1 022/mois</span> sans rien faire d'utile. En les passant sur Gemini Flash-Lite toutes les 55 minutes, on tombe à <span class="badge badge-green">~$1/mois</span>.</p>
      <p class="para">Pourquoi 55 minutes ? Le cache de prompt expire après ~1 heure. Le heartbeat rafraîchit le cache juste avant son expiration.</p>
      <div class="cblock">
        <div class="cblock-header"><span class="cblock-label">Terminal Coolify</span><button class="cblock-copy" onclick="copyEl(this,'code6')">copier</button></div>
        <div class="cblock-code" id="code6">openclaw config set agents.defaults.heartbeat '{"every":"55m","model":"openrouter/google/gemini-2.5-flash-lite","target":"last"}'</div>
      </div>
      <div class="callout callout-success">✅ Économie estimée : $178/mois sur cette seule optimisation.</div>
    </div>
  </div>
  ...
  <div id="step7" class="step">
    <div class="step-header">
      ...
      <div class="step-title">Compaction mémoire</div>
    </div>
    <div class="step-body">
      <p class="para">Chaque échange ajoute des tokens au contexte. La compaction mémoire nettoie automatiquement le contexte avant qu'il ne gonfle trop.</p>
      <div class="cblock">
        <div class="cblock-header"><span class="cblock-label">Terminal Coolify</span><button class="cblock-copy" onclick="copyEl(this,'code7')">copier</button></div>
        <div class="cblock-code" id="code7">openclaw config set agents.defaults.compaction '{"memoryFlush":{"enabled":true,"softThresholdTokens":4000}}'</div>
      </div>
    </div>
  </div>
  ...
  <div id="step8" class="step">
    <div class="step-header">
      ...
      <div class="step-title">Cache de prompts</div>
    </div>
    <div class="step-body">
      <p class="para">Anthropic offre une réduction de 90% sur les tokens mis en cache. Google offre 75%. À vérifier/activer depuis le chat OpenClaw :</p>
      <div class="cblock">
        <div class="cblock-header"><span class="cblock-label">Chat OpenClaw (Telegram ou WebUI)</span><button class="cblock-copy" onclick="copyEl(this,'code8')">copier</button></div>
        <div class="cblock-code" id="code8">Confirme : le cache de prompts est-il activé pour ma configuration de modèles actuelle ? Sinon, active-le. Vérifie également que mon TTL de cache est configuré de manière optimale (doit être 3600 secondes / 1 heure pour les meilleures économies).</div>
      </div>
    </div>
  </div>
  ...
  <div id="step9" class="step">
    <div class="step-header">
      ...
      <div class="step-title">Garde-fous budgétaires</div>
    </div>
    <div class="step-body">
      <p class="para">À envoyer dans le chat OpenClaw pour ajouter des limites automatiques :</p>
      <div class="cblock">
        <div class="cblock-header"><span class="cblock-label">Chat OpenClaw (Telegram ou WebUI)</span><button class="cblock-copy" onclick="copyEl(this,'code9')">copier</button></div>
        <div class="cblock-code" id="code9">Ajoute ces garde-fous budgétaires à mon prompt système :
...
Confirme que ces règles sont maintenant dans mon prompt système.</div>
      </div>
    </div>
  </div>
  ...
  <div id="step10" class="step">
    <div class="step-header">
      ...
      <div class="step-title">Restart et vérification finale</div>
    </div>
    <div class="step-body">
      <p class="para">Faire un <strong>Restart depuis Coolify</strong>, attendre 30 secondes, puis vérifier.</p>
      <div class="cblock">
        <div class="cblock-header"><span class="cblock-label">Vérifier le modèle actif (Telegram)</span><button class="cblock-copy" onclick="copy(this,'/status full')">copier</button></div>
        <div class="cblock-code">/status full</div>
      </div>
      <div class="cblock">
        <div class="cblock-header"><span class="cblock-label">Vérifier la config providers (Terminal Coolify)</span><button class="cblock-copy" onclick="copyEl(this,'code10b')">copier</button></div>
        <div class="cblock-code" id="code10b">cat /data/.openclaw/openclaw.json | grep -A3 "providers"</div>
      </div>
      <div class="cblock">
        <div class="cblock-header"><span class="cblock-label">Voir les coûts et l'usage (Telegram)</span><button class="cblock-copy" onclick="copy(this,'/usage')">copier</button></div>
        <div class="cblock-code">/usage</div>
      </div>
      <div class="callout callout-warn">⚠️ Si providers: {} → relancer l'étape 3 puis restart immédiatement sans envoyer de message.</div>
    </div>
  </div>
  ...
  <!-- REF -->
  <div id="ref" class="step">
    <div class="step-header">
      <div class="step-num">★</div>
      <div class="step-title">Arbre de décision — Quel modèle utiliser ?</div>
    </div>
    <div class="step-body">
      <table class="tbl">
        <thead><tr><th>Si vous faites…</th><th>Alias</th><th>Modèle</th><th>Coût</th></tr></thead>
        <tbody>
          <tr><td>Heartbeat / cron / ping</td><td><span class="code">flashlite</span></td><td>Gemini Flash-Lite</td><td><span class="badge badge-green">$0.18/M</span></td></tr>
          <tr><td>Question du quotidien</td><td><span class="code">deepseek</span></td><td>DeepSeek V3.2 ⭐</td><td><span class="badge badge-green">$0.31/M</span></td></tr>
          <tr><td>Code simple</td><td><span class="code">deepseek</span></td><td>DeepSeek V3.2</td><td><span class="badge badge-green">$0.31/M</span></td></tr>
          <tr><td>Contexte très long</td><td><span class="code">kimi</span></td><td>Kimi K2.5</td><td><span class="badge badge-green">$1.08/M</span></td></tr>
          <tr><td>Code complexe</td><td><span class="code">glm</span></td><td>GLM-5</td><td><span class="badge badge-amber">$1.35/M</span></td></tr>
          <tr><td>Rédaction qualité / recherche</td><td><span class="code">sonnet</span></td><td>Claude Sonnet 4.6</td><td><span class="badge badge-amber">$6/M</span></td></tr>
          <tr><td>Tâche critique / raisonnement complexe</td><td><span class="code">opus</span></td><td>Claude Opus 4.6</td><td><span class="badge badge-red">$10/M</span></td></tr>
          <tr><td>Open-source économique</td><td><span class="code">llama8b</span></td><td>Llama 3.1 8B</td><td><span class="badge badge-green">très faible</span></td></tr>
          <tr><td>Open-source qualité</td><td><span class="code">llama70b</span></td><td>Llama 3.3 70B</td><td><span class="badge badge-green">faible</span></td></tr>
          <tr><td>Contexte 512K open-source</td><td><span class="code">llama4</span></td><td>Llama 4 Scout</td><td><span class="badge badge-green">faible</span></td></tr>
        </tbody>
      </table>

      <div class="h3">Changer de modèle depuis Telegram</div>
      <p class="para"><strong>Méthode 1 — Via alias (rapide)</strong></p>
      <div class="cblock">
        <div class="cblock-header"><span class="cblock-label">Telegram</span><button class="cblock-copy" onclick="copy(this,'/model deepseek')">copier</button></div>
        <div class="cblock-code">/model deepseek</div>
      </div>
      <p class="para"><strong>Méthode 2 — Via le menu interactif</strong></p>
      <p class="para">Taper <span class="code">/models</span> → cliquer sur <strong>openrouter</strong> → choisir le modèle dans la liste.</p>
      <div class="cblock">
        <div class="cblock-header"><span class="cblock-label">Telegram</span><button class="cblock-copy" onclick="copy(this,'/models')">copier</button></div>
        <div class="cblock-code">/models</div>
      </div>
      <div class="screenshots">
        <div class="screenshot"><img src="https://i.imgur.com/XOetcN9.jpeg" alt=""/><div class="screenshot-cap">Étape 1 — liste des providers</div></div>
        <div class="screenshot"><img src="https://i.imgur.com/VFXDM32.jpeg" alt=""/><div class="screenshot-cap">Étape 2 — modèles OpenRouter</div></div>
        <div class="screenshot"><img src="https://i.imgur.com/91iNLiC.jpeg" alt=""/><div class="screenshot-cap">Étape 3 — confirmation</div></div>
      </div>
    </div>
  </div>
  ...
  <div id="prompts" class="step">
    <div class="step-header">
      <div class="step-num">💬</div>
      <div class="step-title">Prompts utiles — Contrôle des coûts</div>
    </div>
    <div class="step-body">
      <div class="h3">Audit des coûts</div>
      <div class="cblock">
        <div class="cblock-header"><span class="cblock-label">Chat OpenClaw</span><button class="cblock-copy" onclick="copyEl(this,'code-audit')">copier</button></div>
        <div class="cblock-code" id="code-audit">/status full
/usage

Ensuite demande-moi : "Quels patterns vois-tu dans mon utilisation API ? Où puis-je encore réduire les coûts ?"</div>
      </div>
      <div class="h3">Règle d'efficacité des sorties d'outils</div>
      <div class="cblock">
        <div class="cblock-header"><span class="cblock-label">Chat OpenClaw</span><button class="cblock-copy" onclick="copyEl(this,'code-rule')">copier</button></div>
        <div class="cblock-code" id="code-rule">Ajoute cette règle à mon prompt système :
...
Confirme l'ajout.</div>
      </div>
      <div class="h3">Arrêt d'urgence</div>
      <div class="cblock">
        <div class="cblock-header"><span class="cblock-label">Telegram — pause tous les appels API pendant 1h</span><button class="cblock-copy" onclick="copy(this,'/ratelimit pause')">copier</button></div>
        <div class="cblock-code">/ratelimit pause</div>
      </div>
    </div>
  </div>

  <!-- DEPANNAGE -->
  <div id="debug" class="step">
    <div class="step-header">
      <div class="step-num">🔧</div>
      <div class="step-title">Dépannage</div>
    </div>
    <div class="step-body">
      <div class="h3">Erreur : Unknown model: openrouter/...</div>
      <p class="para">Le bloc providers a été écrasé. Vérifier :</p>
      <div class="cblock">
        <div class="cblock-header"><span class="cblock-label">Terminal Coolify</span><button class="cblock-copy" onclick="copyEl(this,'code-dbg1')">copier</button></div>
        <div class="cblock-code" id="code-dbg1">cat /data/.openclaw/openclaw.json | grep -A3 "providers"</div>
      </div>
      <p class="para">Si le résultat affiche <span class="code">providers: {}</span> → relancer l'étape 3 puis restart immédiatement.</p>

      <div class="h3">Heartbeats toujours chers</div>
      <div class="cblock">
        <div class="cblock-header"><span class="cblock-label">Terminal Coolify</span><button class="cblock-copy" onclick="copyEl(this,'code-dbg2')">copier</button></div>
        <div class="cblock-code" id="code-dbg2"># */55 * * * * /usr/bin/curl -X POST http://localhost:3000/api/agent/main -H "Content-Type: application/json" -d '{"message":"ping","sessionId":"isolated"}' >/dev/null 2>&1</div>
      </div>

      <div class="h3">Coût toujours élevé après optimisation</div>
      <div class="cblock">
        <div class="cblock-header"><span class="cblock-label">Telegram</span><button class="cblock-copy" onclick="copy(this,'/status full')">copier</button></div>
        <div class="cblock-code">/status full</div>
      </div>
    </div>
  </div>
...
<footer>
  <p>Daemon IA — <a href="https://daemon-ia.fr">daemon-ia.fr</a> — Mars 2026</p>
</footer>
...
function copyEl(btn, id) {
  copy(btn, document.getElementById(id).textContent);
}
...
export default function App() {
  ...
  const activeColor = TABS.find(t => t.id === tab)?.color || "#10b981";
  ...
  return (
    <div style={{minHeight:"100vh",background:"#09090b",fontFamily:"'Inter',system-ui,sans-serif",color:"#fafafa",overflowX:"hidden"}}>
      ...
      {/* Ambient glow radial */}
      <motion.div
        animate={{ background: `radial-gradient(ellipse 80% 50% at 50% -10%, ${activeColor}12 0%, transparent 70%)` }}
        transition={{ duration: 0.8 }}
        style={{position:"fixed",inset:0,pointerEvents:"none",zIndex:0}}
      />

      {/* NAV — ForgeUI style */}
      <nav style={{position:"sticky",top:0,zIndex:50,background:"rgba(9,9,11,0.75)",backdropFilter:"blur(20px) saturate(160%)",borderBottom:"1px solid rgba(255,255,255,0.05)"}}>
        {/* Shimmer top border */}
        <motion.div
          animate={{ background: `linear-gradient(90deg, transparent 0%, ${activeColor}70 50%, transparent 100%)` }}
          transition={{ duration: 0.5 }}
          style={{height:"1px",width:"100%"}}
        />

        <div style={{maxWidth:1400,margin:"0 auto",padding:"0 20px",display:"flex",alignItems:"center",height:58,gap:16}}>

          {/* Logo + wordmark */}
          <div style={{display:"flex",alignItems:"center",gap:8,flexShrink:0}}>
            <motion.div
              animate={{ boxShadow: `0 0 16px ${activeColor}50` }}
              transition={{ duration: 0.5 }}
              style={{width:28,height:28,borderRadius:7,background:`linear-gradient(135deg,${activeColor},${activeColor}99)`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:14,fontWeight:800,color:"#09090b",letterSpacing:"-0.03em",flexShrink:0}}
            >D</motion.div>
            <span style={{fontSize:12,fontWeight:700,color:"rgba(250,250,250,0.3)",letterSpacing:"0.08em",textTransform:"uppercase",flexShrink:0}}>Daemon IA</span>
          </div>
                ...
                <div key={id} style={{position:"relative",flexShrink:0}}>
                  ...
                  {isHov && (
                    <motion.div
                      ...
                      animate={{opacity:1}}
                      exit={{opacity:0}}
                      style={{position:"absolute",inset:0,borderRadius:8,background:"rgba(255,255,255,0.04)"}}
                    />
                  )}
                  {/* Active bg */}
                  {on && (
                    <motion.div
                      layoutId="activeBg"
                      style={{position:"absolute",inset:0,borderRadius:8,background:`${color}12`,border:`1px solid ${color}25`}}
                  ...
                  <button
                    ...
                    style={{
                      ...
                      background:"transparent",
                      color: on ? color : isHov ? "rgba(250,250,250,0.7)" : "rgba(250,250,250,0.38)",
                      outline:"none",transition:"color 0.2s",zIndex:1,
                    }}
                  >
                    {logo && <img src={logo} style={{width:14,height:14,objectFit:"contain",borderRadius:round?"50%":3,flexShrink:0,opacity:on?1:0.5}} alt=""/>}
                    {label}
                  </button>
                  ...
                  {on && (
                    <motion.div
                      layoutId="activeDot"
                      style={{position:"absolute",bottom:-1,left:"50%",transform:"translateX(-50%)",width:20,height:2,borderRadius:"2px 2px 0 0",background:color}}
          ...
          <a
            ...
            style={{flexShrink:0,display:"flex",alignItems:"center",gap:6,padding:"6px 14px",borderRadius:8,background:`${activeColor}14`,border:`1px solid ${activeColor}30`,color:activeColor,fontSize:12,fontWeight:600,textDecoration:"none",transition:"all 0.25s",letterSpacing:"0.01em",whiteSpace:"nowrap"}}
            onMouseEnter={e=>{ e.currentTarget.style.background=`${activeColor}25`; e.currentTarget.style.boxShadow=`0 0 12px ${activeColor}30`; }}
            onMouseLeave={e=>{ e.currentTarget.style.background=`${activeColor}14`; e.currentTarget.style.boxShadow="none"; }}
          >
            <svg width="11" height="11" viewBox="0 0 32 32" fill="currentColor"><path d="M16 2C8.268 2 2 8.268 2 16c0 2.478.677 4.8 1.854 6.793L2 30l7.42-1.822A13.94 13.94 0 0016 30c7.732 0 14-6.268 14-14S23.732 2 16 2z"/></svg>
            Contact
          </a>
        </div>
      </nav>
      ...
      {!isFullscreen && <>
        <motion.a
          ...
          whileTap={{scale:0.95}}
          style={{position:"fixed",bottom:28,right:28,width:50,height:50,borderRadius:"50%",background:"#25D366",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:"0 4px 20px rgba(37,211,102,0.3)",zIndex:100,textDecoration:"none"}}
        >
        ...
        <motion.button
          ...
          whileTap={{scale:0.95}}
          animate={{borderColor:`${activeColor}40`,color:activeColor}}
          transition={{duration:0.3}}
          style={{position:"fixed",bottom:28,right:88,width:50,height:50,borderRadius:"50%",border:`1px solid ${activeColor}30`,background:"rgba(9,9,11,0.7)",backdropFilter:"blur(8px)",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",zIndex:100,fontSize:18,fontWeight:700,color:activeColor}}
        >↑</motion.button>
      </>}
    </div>
  );
